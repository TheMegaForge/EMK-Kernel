...
pub enum Errno {
    ...
    EINT = 0x04,            // Interrupted system call
    ...
    EWOULDBLOCK = 0x0b,     // Resource temporarily unavailable
    ...
    UNUSED29H = 0x29,       // not implemented
    ...
    ENOSCI = 0x32,          // No CSI structure available
    ...
    UNUSED3AH = 0x3a,       //	not implemented
    ...
    EDESTADDRRE = 0x59,     // Destination address required
    ...
    ENOTSUP = 0x5f,         // Operation not supported
    ...
    EDQOUT = 0x7a,          // Disk quota exceeded
...
pub fn to_lowercase(r#in: char) -> char {
    let is_uppercase = r#in as u8 >= 0x41 && 0x5A >= r#in as u8;
    (r#in as u8 + 0x20 * is_uppercase as u8) as char
}
/** length check is c style*/
pub fn unchecked_construct_utf8_string<'a>(memory: *const u8) -> &'a str {
    let mut length = 0;

    while unsafe { *memory.add(length) } != 0 {
        length += 1;
    }

    unsafe { str::from_utf8_unchecked(slice::from_raw_parts(memory, length)) }
}

pub fn c_style_length_check_u32(memory: *const u32) -> u32 {
    let mut length = 0;
...
pub fn c_style_length_check_u64(memory: *const u64) -> u32 {
    let mut length = 0;
